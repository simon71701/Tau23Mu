...
config.General.requestName = 'SkimPhiMuNu_2024era{era}_{version}_stream{stream}_Mini_v1'
...
config.JobType.psetName = '/afs/cern.ch/user/b/bsimon/CMSSW_14_0_18/src/SkimTools/SkimPhiMuNu/test/run_Data2024_DsPhiMuNu_SkimAndTree_cfg.py'
...
config.Data.outputDatasetTag = 'SkimPhiMuNu_2024era{era}_{version}_stream{stream}_Mini_v1'
...
for portion in portions:
    ...
    for i in range(0,8):
        ...
        if a.submit:
            ...
            cfgName = "crab_PhiMuNu_reco2024_Run{era}_{version}_stream{stream}_cfg.py".format(**args)
            with open(cfgName, "w") as text_file:
                text_file.write(crabCfg)

            subprocess.call(["crab", "submit", cfgName])
        if a.resubmit:
           folder = "crab_projects/crab_SkimPhiMuNu_2024era{era}_{version}_stream{stream}_Mini_v1".format(**args)
           print (folder)
